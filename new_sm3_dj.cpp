...
float **s1,**s2,**s3,**p2,**sx11,**sx12,**sx13,**sx21,**dj,**dj_read,***bigmovie,\
**sx22,**sx23,**sx24,**sx31,**sx32,**sx33,**suface_data,*zbbx,**nspy,**nspy2;
char *str_movie,*str_forwave,*str_suface,*str_nspy,*str_id,*str_dj1,*str_dj2,*str_djout;
...
/*在逆时偏移情况下，读入地表记录*/
   ...
   bigmovie=new float**[T];
   for(i=0;i<T;i++)
    {bigmovie[i] = new float*[Y] ;
     for(j=0;j<Y;j++)  
     {  
      bigmovie[i][j] = new float[X] ;
     }
    }
    str_dj1=new char[20];
    str_dj2=new char[5];
    str_djout=new char[20];
    strcpy(str_dj1,"amovie");
    strcpy(str_djout,"dj");
    strcat(str_dj1,str_id);
    strcat(str_djout,str_id);
     if(!infile_djread) cout<<"error"<<endl;
    }
...
-sx33[j][i];}   
    ...
    if(source_model==4)
    {
    infile_djread.open(str_dj1,ios::binary);
    for(k=0;k<T;k++)  
     {for(i1=0;i1<X;i1++)
      {for(j1=0;j1<Y;j1++)
       {
          infile_djread.read(( char *)&dj_read[j1][i1],sizeof(dj_read[j1][i1]));
          dj[j1][i1]=dj[j1][i1]+bigmovie[T-1-k][j1][i1]*dj_read[j1][i1];      
       }
      }
     }
    }
   ...
   if(source_model==4)
    {     
    outf_dj.open(str_djout);
    for(j=0;j<X;j++)
    {for(i=0;i<Y;i++)
     {outf_dj.write((char*)&dj[i][j],sizeof(dj[i][j]));}}
     outf_dj.close();
     infile_djread.close();
